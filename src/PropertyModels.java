...
abstract class AbstractHome implements Renderable {
    ...
    private void applyPlaceholder(ImageView v) {
        Rectangle r = new Rectangle(180, 130);
...
class HomeResult extends AbstractHome {
    ...
    public void renderTo(VBox container) {
        ...
        card.setStyle(
                "-fx-background-color: white; -fx-border-color: #ddd; -fx-border-radius: 12; -fx-background-radius: 12;");
        ...
        boolean isForSale = forSaleVal.equals("1") || forSaleVal.equalsIgnoreCase("true")
                || forSaleVal.equalsIgnoreCase("TRUE");
        ...
        ((Label) titleBox.getChildren().get(0))
                .setStyle("-fx-font-weight: bold; -fx-font-size: 16px; -fx-text-fill: #3f51b5;");

        String ph = PropertyService.generatePhoneNumber();
        Label phLab = new Label("📞 Contact: " + ph);
        phLab.setStyle("-fx-text-fill: #555; -fx-font-style: italic;");
        ...
        if ("1".equals(String.valueOf(rawData.getOrDefault("has_elevator", "0"))))
            amenities.append("🛗 Elevator ");
        if ("1".equals(String.valueOf(rawData.getOrDefault("has_school", "0"))))
            amenities.append("🏫 School ");
        if ("1".equals(String.valueOf(rawData.getOrDefault("has_park", "0"))))
            amenities.append("🌳 Park ");
        if ("1".equals(String.valueOf(rawData.getOrDefault("has_shop", "0"))))
            amenities.append("🛒 Shop ");
        if ("1".equals(String.valueOf(rawData.getOrDefault("has_pharmacy", "0"))))
            amenities.append("💊 Pharmacy ");
        String amenitiesStr = amenities.length() > 0 ? amenities.toString().trim() : "None";

        Label priceLabel = new Label(this.price + " zł (" + rawData.get("score") + "% Match)");
        priceLabel.setStyle("-fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: #ff4081;");
        ...
        info.getChildren().addAll(
                ...
                new Label("✨ Amenities: " + amenitiesStr),
                save);
